...
Page({
  ...
  linkToCart() {
    const { name , retailPrice, listPicUrl} = this.data.detail;
    app.store.cloudApi.addToCart({
      name,
      retailPrice,
      listPicUrl,
      quantity: 1
   ...
   */
  getProductDetail() {
     const item = JSON.parse(this.options.item);
     const { name , retailPrice, listPicUrl} = item;
    this.setData({
      detail:item,
      attrs: {
        name,
